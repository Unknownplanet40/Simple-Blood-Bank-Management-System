            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
                    <div class="navbar-nav ms-auto">
                        <a class="nav-link active" aria-current="page">Administrator Accounts</a>
                        <a class="nav-link" href="Admin_Users_Staff.php">Staff Members Accounts</a>
            ...
            <table class="table table-dark table-hover table-striped caption-top table-bordered">
                <caption>
                    ...
                    <form class="d-flex mb-1 mt-2 justify-content-between" method="post">
                        ...
                        <div class="d-flex justify-content-evenly mb-0 mt-0 bg-dark text-white "
                            ...
                            <div><button type="button" btn-sm class="btn btn-sm btn-light mb-2 mt-2 AdminAdd"
                                    data-bs-toggle="modal" data-bs-target="#AdminAdd">Add Account</button></div>
                        </div>
                    </form>
                </caption>
                ...
                <tbody>
                    ...
                    if (isset($_POST['submit'])) {
                        $search = $_POST['search'];
                        $fromSearch = true;
                        $sql = "SELECT * FROM `users` WHERE `types` = 1 AND `name` LIKE '%$search%' OR `email` LIKE '%$search%' OR `username` LIKE '%$search%'";
                    } else {
                        $sql = "SELECT * FROM `users` WHERE `types` = 1";
                    }
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        if ($num >= 7) {
                            ...
                            for ($btn = 1; $btn <= $total_pages; $btn++) {
                                //Page Button
                                echo "<a class='btn btn-secondary btn-sm mx-1' href='Admin_Users.php?page=" . $btn . "'>" . $btn . "</a>";
                            ...
                            $sql = "SELECT * FROM `users` WHERE `types` = 1 LIMIT " . $startinglimit . ',' . $number_pages;
                            $result = mysqli_query($conn, $sql);
                        }
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            // for user type status [Admin, Staff Member, User]
                            if ($row['types'] == 1) {
                                $status = "Admin";
                            } else if ($row['types'] == 2) {
                                $status = "Staff Member";
                        ...
                        if ($fromSearch) {
                            ...
                            echo "<tr>
                                    <td colspan='9'>
                                        <div class='alert alert-warning' role='alert'>We're sorry, but there are no users yet!
                                        </div>
                                    </td>
                                </tr>";
                        }
