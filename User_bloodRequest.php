                        ...
                        <table class="table caption-top table-dark table-striped">
                            ...
                            <tbody>
                                ...
                                if (isset($_POST['submit'])) {
                                    $search = $_POST['search'];
                                    $sql = "SELECT * FROM `request` WHERE `cby` = '$username' AND `user_id` LIKE '%$search%' OR `name` LIKE '%$search%' OR `email` LIKE '%$search%' OR `blood_type` LIKE '%$search%' OR `Address` LIKE '%$search%' AND NOT `isapproved` = '1'";
                                    $fromSearch = true;
                                } else {
                                    $sql = "SELECT * FROM `request` WHERE `cby` = '$username' AND NOT `isapproved` = '1'";
                                }
                                ...
                                if (mysqli_num_rows($result) > 0) {
                                    ...
                                    if ($num >= 6) {
                                        ...
                                        for ($btn = 1; $btn <= $total_pages; $btn++) {
                                            //Page Button
                                            echo "<a class='btn btn-secondary btn-sm mx-1' href='User_bloodRequest.php?page=" . $btn . "'>" . $btn . "</a>";
                                        }
                                        ...
                                        $sql = "SELECT * FROM `Request` WHERE `cby` = '$username' AND NOT `isapproved` = 1 LIMIT $startinglimit, $number_pages";
                                        $result = mysqli_query($conn, $sql);
                                    }
                                    ...
                                    if ($fromSearch) {
                                        echo "<tr>
                                            <td colspan='9'>
                                                <div class='alert alert-warning' role='alert'>We're sorry, but \"$search\" doesn't match any requests!
                                                </div>
                                            </td>
                                        </tr>";
                                    } else {
