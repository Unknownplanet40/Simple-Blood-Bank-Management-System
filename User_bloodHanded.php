                        ...
                        <table class="table caption-top table-dark table-striped">
                            ...
                            <tbody>
                                ...
                                if (isset($_POST['submit'])) {
                                    $search = $_POST['search'];
                                    $sql = "SELECT * FROM `request` WHERE `cby` = '$username' AND `name` LIKE '%$search%' OR `email` LIKE '%$search%' OR `phone` LIKE '%$search%' OR `Address` LIKE '%$search%' OR `blood_type` LIKE '%$search%' AND isapproved = '1'";
                                    $fromSearch = true;
                                } else {
                                    $sql = "SELECT * FROM `request` WHERE `cby` = '$username' AND isapproved = '1'";
                                }
                                ...
                                if (mysqli_num_rows($result) > 0) {
                                    ...
                                    if ($num >= 6) {
                                        ...
                                        for ($btn = 1; $btn <= $total_pages; $btn++) {
                                            //Page Button
                                            echo "<a class='btn btn-secondary btn-sm mx-1' href='User_bloodHanded.php?page=" . $btn . "'>" . $btn . "</a>";
                                        }
                                        ...
                                        $sql = "SELECT * FROM `request` WHERE `cby` = '$username' AND isapproved = 1 LIMIT $startinglimit, $number_pages";
                                        $result = mysqli_query($conn, $sql);
                                    }
                                    ...
                                    if ($fromSearch) {
                                        ...
                                        echo "<tr>
                                            <td colspan='9'>
                                                <div class='alert alert-warning' role='alert'>We're sorry, but you haven't handed out any blood yet!
                                                </div>
                                            </td>
                                        </tr>";
                                    }
