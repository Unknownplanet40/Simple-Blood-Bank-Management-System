            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
                    <div class="navbar-nav ms-auto">
                        ...
                        <a class="nav-link active" aria-current="page">Users Accounts</a>
            ...
            <table class="table table-dark table-hover table-striped caption-top table-bordered">
                <caption>
                    ...
                    <form class="d-flex mb-1 mt-2 justify-content-between" method="post">
                        ...
                        <div class="d-flex justify-content-evenly mb-0 mt-0 bg-dark text-white "
                            style="min-width: 230px; border-radius: 10px;">
                            ...
                            <div><button type="button" btn-sm class="btn btn-sm btn-light mb-2 mt-2"
                                    data-bs-toggle="modal" data-bs-target="#UserAdd">Add Account</button></div>
                        </div>
                    </form>
                </caption>
                ...
                <tbody>
                    ...
                    if (isset($_POST['submit'])) {
                        $search = $_POST['search'];
                        $fromSearch = true;
                        $sql = "SELECT * FROM `users` WHERE `name` LIKE '%$search%' OR `username` LIKE '%$search%' OR `email` LIKE '%$search%' OR `phone` LIKE '%$search%' OR `address` LIKE '%$search%' OR `blood` LIKE '%$search%'  AND `types` = '3'";
                    } else {
                        $sql = "SELECT * FROM `users` WHERE `types` = '3'";
                    }
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        if ($num >= 7) {
                            ...
                            for ($btn = 1; $btn <= $total_pages; $btn++) {
                                //Page Button
                                echo "<a class='btn btn-secondary btn-sm mx-1' href='Admin_Users_Users.php?page=" . $btn . "'>" . $btn . "</a>";
                            ...
                            $sql = "SELECT * FROM `users` WHERE `types` = '3' LIMIT " . $startinglimit . ',' . $number_pages;
                            $result = mysqli_query($conn, $sql);
                        }
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            if ($row['types'] == 3){
                                echo "<tr>
                                        ...
                                        <td class='text-truncate' style='max-width: 150px; min-width: 100px'>" . convert($row['password'], true) . "</td>
